		...
		if (t.showGuiInspector){
			HTGuiTools.BeginGroup();{
				...
				if (t.allowUIDetection ){
					...
					t.autoUpdatePickedUI = HTGuiTools.Toggle("Auto RunInLogic picked Unity UI", t.autoUpdatePickedUI,true);
					EditorGUI.indentLevel--;
				}
		...
		if (t.showSelectInspector){
			HTGuiTools.BeginGroup();{
				...
				if (t.autoSelect){
					...
					t.autoUpdatePickedObject = HTGuiTools.Toggle( "Auto RunInLogic picked gameobject",t.autoUpdatePickedObject,true);
					EditorGUILayout.Space();
